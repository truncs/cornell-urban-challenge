...
TransmitSignaler::~TransmitSignaler()
{
	...
	*/
	...
	printf("Transmit signaler thread terminated successfully.\n");
...
DWORD WINAPI TransmitCallback(LPVOID lpparam)
{
	...
	*/
	...
	Event* NewEvent = NULL;
	//access to the local map and local road event queues
	...
	while (TheSignaler->IsRunning() == true)
	{
		...
		LocalMapTime = TheCarTime->CurrentCarTime();
		iEventTime = LocalMapTime;
		iNumElements = 0;
		iNumColumns = 0;
		iEventData = NULL;

		//create the event data for both the local map and local road event queues
		NewEvent = new Event();
		NewEvent->SetEventType(iEventType, iEventTime);
		NewEvent->SetEventData(iNumElements, iNumColumns, iEventData);
		TheLocalMapEventQueue->PushEvent(NewEvent);

		NewEvent = new Event();
		NewEvent->SetEventType(iEventType, iEventTime);
		NewEvent->SetEventData(iNumElements, iNumColumns, iEventData);
		TheLocalRoadEventQueue->PushEvent(NewEvent);
